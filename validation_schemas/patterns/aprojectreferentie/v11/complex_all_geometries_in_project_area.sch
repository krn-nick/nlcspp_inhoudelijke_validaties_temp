...
<pattern xmlns ="http://purl.oclc.org/dsdl/schematron" id="v11-all-geometries-in-project-area">
    ...
    <rule context="//nlcs:MSstation | //nlcs:ABeschermingsvlak">
        ...
        <assert id="assert-area-inside-project-area" test="keronic:area-2d-interacts-with-area-2d($area_pos_list, $project_area_pos_list)">
            Area with handle <value-of select="$handle"/> is outside of the project area!
        </assert>

        <assert test="1 = 2">
            <value-of select="keronic:cast-2d-to-3d-array($area_pos_list)"/>
        </assert>
    </rule>
</pattern>